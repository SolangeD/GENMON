	...
	<body>
		<div id="page">
			...
			<div id="margin">
				...
				<?php
					$species_tot=pg_query("SELECT distinct species FROM summary");
					for($a=0;$a<pg_num_rows($species_tot);$a++){
					$specie=pg_fetch_result($species_tot,$a,0);
					echo "<br/><h3>".$specie."</h3><br/>";	
					if(isset($user)){
						if(isset($_POST['show_public']) && $_POST['show_public']==1) {
							$sql_summary="SELECT * FROM summary where owner='".$user."' or public=1 and species='".$specie."' order by index_final";
						}
						else{
							$sql_summary="SELECT * FROM summary where owner='".$user."' and species='".$specie."' order by index_final"; 
						}
					}
					else {
						$sql_summary="SELECT * FROM summary where public=1 and species='".$specie."' order by index_final";
					}
